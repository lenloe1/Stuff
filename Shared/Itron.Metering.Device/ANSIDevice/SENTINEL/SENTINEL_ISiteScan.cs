...
{
    /// <summary>
    /// Class representing the SENTINEL. (ISiteScan implementation)
    /// </summary>
    /// Revision History	
    ...
    public partial class SENTINEL : ANSIMeter, ISiteScan, IERTCheck
    {
        ...
        CDiagnostics ISiteScan.Diagnostics
        {
            ...
            {
                CDiagnostics Diag;
                float fFWRev = FWRevision;
                PSEMResponse Result = PSEMResponse.Err;
                byte[] Data = null;
                LID[] LIDs = new LID[8];

                //Create the diagnostic object based on the firmware revision
                if (fFWRev >= (float)5.0)
                {
                    Diag = new CDiagnostics(true);
                }
                else
                {
                    Diag = new CDiagnostics(false);
                }
                ...
                if (fFWRev >= (float)5.0)
                {
                    //Get the Diag 6 count
                    Result = m_lidRetriever.RetrieveLID(m_LID.SITESCAN_DIAG_6_COUNT, out Data);
                    if (PSEMResponse.Ok == Result)
                    {
                        Diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_6].Count = Data[0];
                    }
                }
		...
		/// <summary>
		/// Proporty can be used to determine if snapshots are supported.  The Sentinel
		/// only supported snapshot in saturn or higher firmware.
		/// </summary>
		//  Revision History
		//  MM/DD/YY Who Version Issue# Description
		//  -------- --- ------- ------ ---------------------------------------------
		...
		List<SnapshotEntry> ISiteScan.SiteScanSnapshots
		{
			get
			{
				if (FWRevision >= SENTINEL_SATURN_FW_REV)
				{
					...
					Table2088 table2088 = new Table2088(m_PSEM);
					Table2089 table2089 = new Table2089(m_PSEM, table2088, bPQSupported, true);
					...
					foreach (Table2089Snapshot Table2089SnapshotData in Table2089SnapshotList)
					{
						...
						SnapshotData.PeakCurrentSupported = true;
						SnapshotData.PeakDemandCurrent = (double)table2088.SnapshotPeakDemandCurrent;
						...
						SnapshotData.IRMSSupported = true;
						SnapshotData.IRMS = ConvertCurrent(table2088.SnapshotMeterClass, Table2089SnapshotData.IRMS);
        ...
        /// <summary>
        /// This method decipher the diag byte to determine if any of the 
        /// diagnosics are active.
        /// </summary>
        /// <remarks>
        /// Diagnostics 5a, 5b, 5c are not returned by the meter only 5T.
        /// </remarks>
        /// <param name="byDiagActive">The type of diag active byte passed in</param>
        /// <param name="diag">The diagnostic object</param>
        //  Revision History	
        //  MM/DD/YY who Version Issue# Description
        //  -------- --- ------- ------ ---------------------------------------
        //  05/24/06 mrj 7.30.00 N/A    Created
        //  	
        protected void CheckActiveDiags(byte byDiagActive, ref CDiagnostics diag)
        {
            if (SS_DIAG_1_MASK == (byte)(byDiagActive & SS_DIAG_1_MASK))
            {
                //Diag 1 Active
                diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_1].Active = true;
            }
            if (SS_DIAG_2_MASK == (byte)(byDiagActive & SS_DIAG_2_MASK))
            {
                //Diag 2 Active
                diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_2].Active = true;
            }
            if (SS_DIAG_3_MASK == (byte)(byDiagActive & SS_DIAG_3_MASK))
            {
                //Diag 3 Active
                diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_3].Active = true;
            }
            if (SS_DIAG_4_MASK == (byte)(byDiagActive & SS_DIAG_4_MASK))
            {
                //Diag 4 Active
                diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_4].Active = true;
            }
            if (SS_DIAG_5_MASK == (byte)(byDiagActive & SS_DIAG_5_MASK))
            {
                //Diag 5 Active
                diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_5T].Active = true;
            }

            if (diag.m_Diags.Length == (int)CDiagnostics.DiagEnum.NUM_DIAGS)
            {
                //Diagnostic six is enabled so check the status
                if (SS_DIAG_6_MASK == (byte)(byDiagActive & SS_DIAG_6_MASK))
                {
                    //Diag 6 Active
                    diag.m_Diags[(int)CDiagnostics.DiagEnum.DIAG_6].Active = true;
                }
            }
        }
