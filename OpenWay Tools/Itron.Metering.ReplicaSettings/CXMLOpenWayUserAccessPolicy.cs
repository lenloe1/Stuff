...
namespace Itron.Metering.ReplicaSettings
{
	...
	/// </summary>
	public class CXMLOpenWayUserAccessPolicy : Itron.Metering.ReplicaSettings.CXMLEncryptedSettingsAccess
	{
		#region Constants

		private const string REPLICA_REG_KEY = "OpenWay Replica";
		private const string XML_REPLICA_FILE_NAME = "OpenWayUserAccessPolicy.xml";
        ...
        private const string XML_CREATE_REPLICA_FILE_TAG = "CreateReplicaFile";
        ...
        private const string XML_CLEAR_FWDL_EVENT_LOG = "ClearFWDLEventLog";

        private const string OPENWAY_TOOLS_ADMINISTRATORS = "OpenWay Tools Administrators";
        private const string OPENWAY_TOOLS_POWER_USERS = "OpenWay Tools Power Users";
        private const string OPENWAY_TOOLS_USERS = "OpenWay Tools Users";
        private const string OPENWAY_NORMAL_USERS_2 = "OpenWay Normal Users 2";
        private const string OPENWAY_NORMAL_USERS_3 = "OpenWay Normal Users 3";

        private const string OPENWAY_MEASUREMENT_CANADA_USERS = "OpenWay Measurement Canada Authorized Users";
        ...
        public enum FunctionalCapability
        {
            ...
            ManageDataFiles,
            /// <summary>
            /// Comm Module Operations
            /// </summary>
            CommModuleOperations,
            /// <summary>
            /// HAN Operations
            /// </summary>
            HANOperations,
            ...
            EnterExitTestMode,
            /// <summary>
            /// Clear Firmware Download Event Log
            /// </summary>
            ClearFWDLEventLog,
            /// <summary>
            /// Create Replica File
		...
		public void UpdateAllGroupSIDs()
		{
			try
			{
				...
				{
					String strGroupName = strDomainName + @"\" + OPENWAY_TOOLS_ADMINISTRATORS;
					StoreGroupSID(myPrincipal, strGroupName, OPENWAY_TOOLS_ADMINISTRATORS);

					strGroupName = strDomainName + @"\" + OPENWAY_TOOLS_POWER_USERS;
					StoreGroupSID(myPrincipal, strGroupName, OPENWAY_TOOLS_POWER_USERS);

					strGroupName = strDomainName + @"\" + OPENWAY_TOOLS_USERS;
					StoreGroupSID(myPrincipal, strGroupName, OPENWAY_TOOLS_USERS);

                    strGroupName = strDomainName + @"\" + OPENWAY_NORMAL_USERS_2;
                    StoreGroupSID(myPrincipal, strGroupName, OPENWAY_NORMAL_USERS_2);

                    strGroupName = strDomainName + @"\" + OPENWAY_NORMAL_USERS_3;
                    StoreGroupSID(myPrincipal, strGroupName, OPENWAY_NORMAL_USERS_3);
				}
			}
			catch
			{
        ...
        public bool VerifiedOpenWayUser
        {
            get
            {
                return VerifyUser(UserAccessPolicy, new List<string>() 
                    { OPENWAY_TOOLS_ADMINISTRATORS, OPENWAY_TOOLS_POWER_USERS, OPENWAY_TOOLS_USERS, 
                        OPENWAY_NORMAL_USERS_2, OPENWAY_NORMAL_USERS_3 });
            }
        }
        ...
        public bool VerifiedOpenWayAdministrator
        {
            get
            {
                ...
                //is in force or not.
                return VerifyUser(true, new List<string>() { OPENWAY_TOOLS_ADMINISTRATORS });
            }
        }

        /// <summary>
        /// Gets whether the currenly logged on user is an OpenWay Measurement Canada user.
        /// </summary>
        //  Revision History
        //  MM/DD/YY Who Version  Issue#    Description
        //  -------- --- -------  --------- ---------------------------------------------
        //  08/24/15 PGH 4.50.198 REQ574469 Created
        //
        public bool VerifiedOpenWayMeasurementCanada
        {
            get
            {
                return VerifyUser(true, new List<string>() { OPENWAY_MEASUREMENT_CANADA_USERS });
            }
        }
		...
		private List<string> GetUserGroups(string strXMLTag)
		{
            ...
            lststrRoles.Insert(0, OPENWAY_TOOLS_ADMINISTRATORS);
		...
		private static String GetSIDName(String strGroupName)
		{
			...
			if ( strGroupName.Contains ( OPENWAY_TOOLS_ADMINISTRATORS ))
			{
				strSIDName = "AdminGroup";
			}
			else if ( strGroupName.Contains( OPENWAY_TOOLS_POWER_USERS ))
			{
				strSIDName = "PowerGroup";
			}
			else if ( strGroupName.Contains( OPENWAY_TOOLS_USERS ))
			{
				strSIDName = "UserGroup";
			}
            else if (strGroupName.Contains(OPENWAY_NORMAL_USERS_2))
            {
                strSIDName = "UserGroup2";
            }
            else if (strGroupName.Contains(OPENWAY_NORMAL_USERS_3))
            {
